...
class SwinTransformer(Backbone):
    ...
    def __init__(
        ...
        is_new_stage, stage_i = True, 0
        for i_layer in range(self.num_layers):

            # afaptive feedback prompt (AFP) in PAT
            if self.afp and is_new_stage:
                is_new_stage = False
                afp_block = None
                if self.afp_stages is not None and stage_i in self.afp_stages:
                    dim = int(embed_dim * 2**i_layer)
                    # afp_block = PromptBlock(in_shapes=feedback_shapes[stage_i], out_shapes=torch.Size((-1, -1, dim)))
                    afp_block = PromptBlock(in_shapes=torch.Size((-1, -1, 256)), out_shapes=torch.Size((-1, -1, dim)))
                self.afps.append(afp_block)
    ...
    def forward(self, x, afp_feedback=None):
        ...
        for i in range(self.num_layers):
            if self.afp and afp_feedback is not None and is_new_stage:
                is_new_stage = False
                if self.afps[stage_i] is not None:
                    x = self.afps[stage_i](x, Wh, Ww, afp_feedback[stage_i])
...
def build_swint_backbone(cfg, input_shape):
    ...
    # Add prompt tuning block if afp is set to True
    return SwinTransformer(
        ...
        afp=cfg.KD.TYPE in "PAT",
        afp_stages=cfg.KD.PAT.STAGE,
    )
