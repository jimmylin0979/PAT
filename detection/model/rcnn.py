...
def rcnn_dkd_loss(stu_predictions, tea_predictions, gt_classes, alpha, beta, temperature):
    ...
    loss_dkd = dkd.dkd_loss(stu_logits, tea_logits, gt_classes, alpha, beta, temperature)
    return {
        "loss_dkd": loss_dkd,
    }


def rcnn_kd_loss(stu_predictions, tea_predictions, gt_classes, temperature):
    stu_logits, stu_bbox_offsets = stu_predictions
    tea_logits, tea_bbox_offsets = tea_predictions
    gt_classes = torch.cat(tuple(gt_classes), 0).reshape(-1)
    loss_kd = dkd.kd_loss(stu_logits, tea_logits, gt_classes, temperature)
    return {
        "loss_kd": loss_kd,
    }
...
class RCNNKD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.kd_args.KD.TYPE in ("ReviewKD", "ReviewDKD"):
            self.kd_trans = reviewkd.build_kd_trans(self.kd_args)
        elif self.kd_args.KD.TYPE in ("OFA"):
            self.kd_trans = ofakd.build_kd_trans(self.kd_args)
        elif self.kd_args.KD.TYPE in ("PAT"):
            self.kd_trans = pat.build_kd_trans(self.kd_args)
            self.afp_feedback = None
        elif self.kd_args.KD.TYPE in ("FitNet"):
            self.kd_trans = fitnet.build_kd_trans(self.kd_args)
    ...
    def forward_pure_roi_head(self, roi_head, features, proposals, return_box_features=False):
        ...
        # set return_box_features for OFA
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        if self.kd_args.KD.TYPE == "KD":
            ...
            detector_losses.update(
                rcnn_kd_loss(
                    ...
                    self.kd_args.KD.KD.T,
                )
            )
        elif self.kd_args.KD.TYPE == "DKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            detector_losses.update(
                rcnn_dkd_loss(
                    ...
                    tea_predictions,
                    [x.gt_classes for x in sampled_proposals],
                    self.kd_args.KD.DKD.ALPHA,
                    self.kd_args.KD.DKD.BETA,
                    self.kd_args.KD.DKD.T,
                )
            )

        elif self.kd_args.KD.TYPE == "ReviewKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            t_features = [t_features[f] for f in t_features]
            s_features = [features[f] for f in features]
            s_features = self.kd_trans(s_features)
            losses["loss_reviewkd"] = reviewkd.hcl(s_features, t_features) * self.kd_args.KD.REVIEWKD.LOSS_WEIGHT

        elif self.kd_args.KD.TYPE == "ReviewDKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            # dkd loss
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
        ...
        elif self.kd_args.KD.TYPE == "OFA":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions, stu_box_features = self.forward_pure_roi_head(
                self.roi_heads, features, sampled_proposals, return_box_features=True
            )
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            ...
            t_features = [t_features[f] for f in t_features]
            s_features = [features[f] for f in features]
            labels = [x.gt_classes for x in sampled_proposals]
            detector_losses.update(
                self.kd_trans(stu_predictions, stu_box_features, tea_predictions, t_features, labels)
            )
            # raise NotImplementedError("OFA is not implemented for detection yet")

        elif self.kd_args.KD.TYPE == "PAT":

            # forward on student model
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)

            # forward on teacher model
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features_woPrompt = self.teacher.backbone(teacher_images.tensor)  # [bs, c, h, w]
            tea_predictions_woPrompt = self.forward_pure_roi_head(
                self.teacher.roi_heads, t_features_woPrompt, sampled_proposals
            )

            # forward on teacher model w/ AFP
            t_features = self.teacher.backbone(teacher_images.tensor, afp_feedback=self.afp_feedback)
            self.afp_feedback = None
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)

            #
            t_features_woPrompt = [t_features[f] for f in t_features]
            t_features = [t_features[f] for f in t_features]
            s_features = [features[f] for f in features]
            labels = [x.gt_classes for x in sampled_proposals]

            afp_feedback, losses_dict = self.kd_trans(
                stu_predictions, s_features, tea_predictions_woPrompt, tea_predictions, t_features, labels
            )
            self.afp_feedback = afp_feedback
            detector_losses.update(losses_dict)

        elif self.kd_args.KD.TYPE == "FitNet":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            ...
            t_features = [t_features[f] for f in t_features]
            s_features = [features[f] for f in features]
            losses_dict = self.kd_trans(stu_predictions, s_features, tea_predictions, t_features)
            detector_losses.update(losses_dict)

        else:
            raise NotImplementedError(self.kd_args.KD.TYPE)
