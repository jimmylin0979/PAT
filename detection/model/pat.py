...
def build_kd_trans(cfg):
    #
    # suppose the input is [bs, 3, 800, 1216]
    ...
    model = PAT(student_shapes, teacher_shapes, criterion, cfg)
    return model
...
class PAT(nn.Module):
    def __init__(self, student_shapes, teacher_shapes, criterion, cfg, **kwargs):
        super(PAT, self).__init__()

        #
        self.pat_stage = cfg.KD.PAT.STAGE
        self.pat_num_queries = cfg.KD.PAT.RAA_NUM_QUERIES
        self.pat_dim = cfg.KD.PAT.RAA_DIM
        self.pat_mse_loss_weight = cfg.KD.PAT.MSE_LOSS_WEIGHT
        self.pat_reg_loss_weight = cfg.KD.PAT.REG_LOSS_WEIGHT
        self.gt_loss_weight = cfg.KD.PAT.GT_LOSS_WEIGHT
        self.kd_loss_weight = cfg.KD.PAT.KD_LOSS_WEIGHT
        self.temperature = cfg.KD.PAT.TEMPERATURE
        ...
        # # TODO: Get the max size during training, pad the smaller image to the same size
        # c, h, w = 255, 1333, 1333  # cfg.KD.pat.INPUT_SIZE
        # x = torch.randn(1, c, h, w).cuda()
        # prompt_shapes = self.forward_teacher_with_prompts(x, None, None, True)

        # prompt_blocks = []
        # for stage_i, prompt_shape in enumerate(prompt_shapes):
        #     # print(teacher_shapes[stage_i], prompt_shape)
        ...
        num_queries = self.pat_num_queries  # num_queries = num_stages(4) * num_queries_per_stage
        dim = self.pat_dim  # dim = 512
        ...
        num_stages = len(self.pat_stage)
        self.aligners = nn.ModuleList()
        for stage_i, stage in enumerate(self.pat_stage):
            # idx_t, _ = self.teacher.stage_info(stage)
            # idx_s, _ = self.student.stage_info(stage)

            # just a simple alignment block
            module = get_projector_module([1, int(num_queries / num_stages), dim], teacher_shapes[stage_i])
        ...
        if isinstance(self.teacher, timm.models.vision_transformer.VisionTransformer):
            ...
            for blk in self.teacher.blocks:
                ...
                if stage_idx == -1 or feat_idx == stage_idxs[stage_idx] + 1:
                    ...
                    x = self.forward_with_prompts(x, stage_idx + 1, feat_student, feat_teacher)
                    stage_idx += 1
    ...
    def forward(
        self,
        pred_student: Tuple[torch.Tensor, torch.Tensor],
        feat_student: List[torch.Tensor],
        pred_teacher_woPrompt: Tuple[torch.Tensor, torch.Tensor],
        pred_teacher: Tuple[torch.Tensor, torch.Tensor],
        feat_teacher: List[torch.Tensor],
        label,
        *args,
        **kwargs
    ):
        ...
        logits_teacher_woPrompt, _ = pred_teacher_woPrompt

        ### pat module forward ###
        # extract the stage feature from teacher & student
        feat_s, feat_t = feat_student, feat_teacher
        # for stage in self.pat_stage:
        #     idx_t, _ = self.teacher.stage_info(stage)
        #     idx_s, _ = self.student.stage_info(stage)
        ...
        loss_pat_mse = 0.0
        prev_feat_student, prev_feat_teacher, afp_feedback = [], [], []
        for stage in range(len(feat_t)):
            feat_s_aligned = self.aligners[stage](feat_s_blended[stage].squeeze(0))
            feat_t_i = feat_t[stage]
            feat_t_i = F.interpolate(feat_t_i, size=(self.teacher_shapes[stage][-2], self.teacher_shapes[stage][-1]))
            ...
            afp_feedback.append(prev_feat_student[-1] - prev_feat_teacher[-1])
        ...
        loss_pat_mse = self.pat_mse_loss_weight * loss_pat_mse
        loss_pat_reg = self.pat_reg_loss_weight * kd_loss(logits_teacher, logits_teacher_woPrompt)
        # loss_gt is already included in detectron2
        # loss_gt = self.gt_loss_weight * self.criterion(logits_student, label)
        loss_kd = self.kd_loss_weight * kd_loss(logits_student, logits_teacher_woPrompt, self.temperature)
        losses_dict = {
            # "loss_gt": loss_gt,
            "loss_kd": loss_kd,
        ...
        return afp_feedback, losses_dict
...
class ViT2CNNProjector(nn.Module):
    ...
    def forward(self, embed: torch.Tensor):
        ...
        embed = torch.nn.functional.interpolate(embed, size=(self.h, self.w), mode="nearest")  # (b, d, h, w)
...
class RAProjector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.nn.functional.interpolate(x, size=self.img_size, mode="nearest")
...
class RegionAttention(nn.Module):
    ...
    def __init__(
        ...
        for i, feat_shape in enumerate(feat_student_shapes):
            ...
            module = RAProjector(
                mapped_feat_student_shapes[i], patch_length=self.num_queries_per_stage, embed_dim=dim, is_q=False
            )
